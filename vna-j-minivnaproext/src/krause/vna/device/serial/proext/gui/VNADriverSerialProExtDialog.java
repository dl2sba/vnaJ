...
public class VNADriverSerialProExtDialog extends VNADriverDialog {
	...
	public VNADriverSerialProExtDialog(VNAMainFrame pMainFrame, IVNADriver pDriver) {
		...
		setTitle(VNADriverSerialProExtMessages.getString("Dialog.title"));
		setDefaultCloseOperation(JDialog.DO_NOTHING_ON_CLOSE);
		setProperties(config);
		setConfigurationPrefix("VNADriverSerialProExtDialog");
		...
		lblPhaseMax = new JLabel(VNADriverSerialProExtMessages.getString("Dialog.lblMin"));
		...
		//
		lblPhaseMin = new JLabel(VNADriverSerialProExtMessages.getString("Dialog.lblPhase"));
		...
		//
		lblOpenTimeOut = new JLabel(VNADriverSerialProExtMessages.getString("VNADriverSerialProExtDialog.lblOpenTimeout.text"));
		...
		//
		lblCommandDelay = new JLabel(VNADriverSerialProExtMessages.getString("VNADriverSerialProExtDialog.lblCommandDelay.text"));
		...
		//
		lblReadTimeout = new JLabel(VNADriverSerialProExtMessages.getString("VNADriverSerialProExtDialog.lblReadTimeout.text"));
		...
		//
		lblReference = new JLabel(VNADriverSerialProExtMessages.getString("VNADriverSerialProExtDialog.lblReference.text")); //$NON-NLS-1$
		...
		btnReset = new JButton(VNADriverSerialProExtMessages.getString("VNADriverSerialProExtDialog.btnDefaults.text")); //$NON-NLS-1$
	...
	protected void doOK() {
		...
		int frqCalVal = IntegerValidator.parse(txtTicks.getText(), 900000, 1100000, VNADriverSerialProExtMessages.getString("Dialog.lblDDSTicks.text"), results);
		int openTimeout = IntegerValidator.parse(txtOpenTimeout.getText(), 500, 99000, VNADriverSerialProExtMessages.getString("VNADriverSerialProExtDialog.lblOpenTimeout.text"), results);
		int readTimeout = IntegerValidator.parse(txtReadTimeout.getText(), 500, 99000, VNADriverSerialProExtMessages.getString("VNADriverSerialProExtDialog.lblReadTimeout.text"), results);
		int commandDelay = IntegerValidator.parse(txtCommandDelay.getText(), 50, 99000, VNADriverSerialProExtMessages.getString("VNADriverSerialProExtDialog.lblCommandDelay.text"), results);
		int steps = IntegerValidator.parse(txtSteps.getText(), 200, 25000, VNADriverSerialProExtMessages.getString("Dialog.lblNoOfSteps.text"), results);
		long min = LongValidator.parse(txtFreqMin.getText(), 1, 999999999999l, VNADriverSerialProExtMessages.getString("Dialog.lblFreq"), results);
		long max = LongValidator.parse(txtFreqMax.getText(), 1, 999999999999l, VNADriverSerialProExtMessages.getString("Dialog.lblFreq"), results);

		if (results.isEmpty()) {
			steps = (steps / 100) * 100;
			dib.setNumberOfSamples4Calibration(steps);
			dib.setDdsTicksPerMHz(frqCalVal);
			...
			dib.setMinFrequency(min);
			dib.setMaxFrequency(max);
