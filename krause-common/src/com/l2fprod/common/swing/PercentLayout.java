...
/**
 * PercentLayout. <BR>
 * Constraint based layout which allow the space to be splitted using percentages. The following are allowed when adding components
 * to container:
 * <ul>
 ...
 * in this case, the component will have a width (or height) of 25 % of the container width (or height) <br>
 * <li>container.add(component, "*"); <br>
 * in this case, the component will take the remaining space. if several components use the "*" constraint the space will be divided
 * among the components.
 * </ul>
 * 
 * @javabean.class name="PercentLayout" shortDescription="A layout supports constraints expressed in percent."
 */
public class PercentLayout implements LayoutManager2 {
	...
	private Hashtable<Component, Constraint> componentToConstraint;
	...
	public Constraint getConstraint(Component component) {
		return this.componentToConstraint.get(component);
	}

	public void setConstraint(Component component, Object constraints) {
		if (constraints instanceof Constraint) {
			this.componentToConstraint.put(component, (Constraint) constraints);
	...
	public void layoutContainer(Container parent) {
		...
		int totalSize = (HORIZONTAL == this.orientation ? d.width : d.height) - (components.length - 1) * this.gap;
		...
		for (int i = 0, c = components.length; i < c; i++) {
			if (components[i].isVisible()) {
				Constraint constraint = this.componentToConstraint.get(components[i]);
				if (constraint == null || constraint == PREFERRED_SIZE) {
					sizes[i] = (HORIZONTAL == this.orientation ? components[i].getPreferredSize().width : components[i].getPreferredSize().height);
		...
		int currentOffset = (HORIZONTAL == this.orientation ? insets.left : insets.top);
