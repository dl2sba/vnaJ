...
public class VNADriverSerialStd2 extends VNADriverSerialBase {
	...
	public VNADriverSerialStd2() {
		final String methodName = "VNADriverSerialStd2";
		TraceHelper.entry(this, methodName);

		setMathHelper(new VNADriverSerialStd2MathHelper(this));
		setDeviceInfoBlock(new VNADriverSerialStd2DIB());
		//
	...
	public void init() throws InitializationException {
		...
		super.init();
		//
		VNADriverSerialStd2DIB dib = (VNADriverSerialStd2DIB) getDeviceInfoBlock();
	...
	private VNABaseSample[] receiveRawMessage(long frequency, int pNumSamples, long frequencyStep, IVNABackgroundTaskStatusListener pListener) throws ProcessingException {
		...
		for (int i = 0; i < pNumSamples; ++i) {
			...
			tempSample.setAngle((buffer[offset] & 0x000000ff) + 256.0d * (buffer[offset + 1] & 0x000000ff));
			tempSample.setLoss((buffer[offset + 2] & 0x000000ff) + 256.0d * (buffer[offset + 3] & 0x000000ff));
	...
	public void showDriverDialog(VNAMainFrame pMF) {
		final String methodName = "showDriverDialog";
		TraceHelper.entry(this, methodName);
	...
	public void startGenerator(long frequencyI, long frequencyQ, int attenuationI, int attenuationQ, int phase, int mainAttenuation) throws ProcessingException {
		...
		TraceHelper.entry(this, methodName, "f=%d", frequencyI);
		if (getPort() != null) {
			flushInputStream();
			sendAsAsciiString("0");
